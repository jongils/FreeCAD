 ...
 *   3. `WriterIges::write(hDoc)`
 *      — 완성된 문서를 IGESCAFControl_Writer로 IGES 파일에 씁니다
 *
 * ## 내부 동작
 * - `IGESCAFControl_Writer::Transfer(hDoc)` 로 XCAF 문서를 IGES 엔터티로 변환
 * - 헤더(Author/Company/Product)는 Part::Interface 설정에서 읽습니다
 * - `writer.Write(filename)` 로 파일 저장; 실패 시 Base::FileException 발생
 */
class ImportExport WriterIges
{
public:
    /// @param file 출력 IGES 파일 경로 (Base::FileInfo 래퍼)
    explicit WriterIges(const Base::FileInfo& file);

    /**
     * XCAF 문서를 IGES 형식으로 파일에 씁니다.
     *
     * @param hDoc  ExportOCAF2::exportObjects() 가 채운 TDocStd_Document 핸들.
     *              문서 안의 형상·색상 정보가 IGES 엔터티로 변환됩니다.
